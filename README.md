...
- Both LLM endpoint and daemon need different ports (LLM: 8000 or 1234, daemon: 8001)
- Note: AIM (AMD Inference Microservice) requires Instinct data center GPU hardware and is not supported locally
- Use case: Development, testing without cluster access, local experimentation

## Quick Start Guide (Recommended for testing without a robot)

Start an LLM endpoint: 1) AIM endpoint 2) local LLM on edge client.
